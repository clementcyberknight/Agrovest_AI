...
async function handleApiRequest(req, res, apiUrl, errorMessage) {
  return cors(req, res, async () => {
    ...
    try {
      ...
      return res.status(200).send({
        success: true,
        message: `${errorMessage} fetched successfully!`,
        data: data,
      });
    } catch (error) {
      logger.error(`Error fetching ${errorMessage}:`, error);
      return res.status(500).send({ error: `Something went wrong while fetching ${errorMessage}.` });
    }
  });
}
...
exports.getWeather = functions.https.onRequest(async (req, res) => {
  ...
  await handleApiRequest(req, res, apiUrl, 'Weather data');
});
...
exports.getSoilInformation = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    ...
    if (!lat || !lon || !property) {
      return res.status(400).send({ error: "Latitude, longitude, and property are required." });
    }
    ...
    try {
      ...
      return res.status(500).send({ error: "Something went wrong while fetching soil data." });
    }
  });
});
